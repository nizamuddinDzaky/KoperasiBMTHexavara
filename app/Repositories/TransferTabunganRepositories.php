    ...
    */
    public function pengajuanTransferAntarTabungan($data)
    {
        ...
        try {
            ...
            if(json_decode($tabungan->detail)->saldo > $nominal)
            {
            ...
            elseif(json_decode($tabungan->detail)->saldo < $nominal)
            {
                DB::rollback();
                $response = array("type" => "error", "message" => "Pengajuan Transfer Antar Tabungan Gagal Dibuat. Saldo tabungan anda tidak cukup.");    
            }
            elseif(json_decode($tabungan->detail)->saldo < json_decode($rekening->detail)->saldo_min)
            {
                DB::rollback();
