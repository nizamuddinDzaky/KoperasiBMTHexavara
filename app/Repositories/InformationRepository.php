...
//    LAPORAN
    ...
    function BukuBesar(Request $request){
        ...
        $dt = PenyimpananBMT::select('penyimpanan_bmt.id','bmt.id_bmt as id_rek','penyimpanan_bmt.id_bmt','penyimpanan_bmt.id_user','users.nama as nama_user','penyimpanan_bmt.status','penyimpanan_bmt.status'
            ...
            ->whereDate('penyimpanan_bmt.created_at',">=",$date['prev'])
            ->whereDate('penyimpanan_bmt.created_at',"<=",$date['now'])
            ->join('bmt','bmt.id','penyimpanan_bmt.id_bmt')
    ...
    function BukuBesar_(Request $request){
        ...
        $dt = PenyimpananBMT::select('penyimpanan_bmt.id','bmt.id_bmt as id_rek','penyimpanan_bmt.id_bmt','penyimpanan_bmt.id_user','users.nama as nama_user','penyimpanan_bmt.status','penyimpanan_bmt.status'
            ...
            ->where('penyimpanan_bmt.id_bmt',$d['id'])
            ->where('penyimpanan_bmt.status', '!=', 'Setoran Awal')
            ->whereDate('penyimpanan_bmt.created_at',">=",$request->startdate)
            ->whereDate('penyimpanan_bmt.created_at',"<=",$request->enddate)
            ->join('bmt','bmt.id','penyimpanan_bmt.id_bmt')
            ->join('users','users.id','penyimpanan_bmt.id_user')
            ->orderBy('penyimpanan_bmt.id')
            ->get();
...
//            ->join('rekening', 'rekening.id', '=', 'pengajuan.id_rekening')
    ...
    if($bmt->save())
    {
        ...
        if($bmt2->save())
        {
            ...
            $penyimpananMaal->status = "Pencairan Donasi";
            $penyimpananMaal->id_maal = $request->idkegiatan;
            $penyimpananMaal->id_donatur = Auth::user()->id;
    ...
    function pencairanDonasiZis($request){

        DB::beginTransaction();
        $danaSosial= BMT::where('id' , 334 )->first();
        $rekeningPenyeimbang = BMT::where('id_rekening', $request->dari)->first();
        $jumlahPencairan = floatval(str_replace(',',"",$request->jumlahPencairan));
        ...
        $bmt->id_user=Auth::user()->id;
        $bmt->id_bmt=$danaSosial->id;
        $bmt->status="pencairan donasi zis";
        $bmt->teller=Auth::user()->id;
        $detail = [
            'jumlah' => $jumlahPencairan,
            'saldo_awal' => floatval($danaSosial->saldo),
        ...
        if($bmt->save())
        {
            ...
            if($bmt2->save())
            {
                ...
                //kurangi dana di saldo dana sosial
                $saldoDanaSosialSekarang = $danaSosial->saldo - $jumlahPencairan;
                $updateRekeningPenyeimbang = BMT::where('id', 334)->update(['saldo' => $saldoDanaSosialSekarang]);
